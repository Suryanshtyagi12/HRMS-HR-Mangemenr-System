...
export default function ManagerTeamPage() {
  ...
  const { data: employeesData, isLoading: loading } = useEmployees({ search: searchQuery });
  const [taskModalOpen, setTaskModalOpen] = useState(false);
  
  const employees = Array.isArray(employeesData) ? employeesData : (employeesData?.items || employeesData?.employees || []);
  
  // For demo: Filter out CEO/Executives and only show 6 employees max (preferring Engineers)
  const teamMembers = employees.filter((emp: any) => {
    const role = (emp.designation || '').toLowerCase();
    return !role.includes('ceo') && !role.includes('chief');
  });
  
  const displayEmployees = teamMembers.slice(0, 6);
    ...
    <RoleGuard allowedRoles={['ADMIN', 'HR_RECRUITER', 'SENIOR_MANAGER']}>
      <div className="p-4 md:p-6 max-w-7xl mx-auto space-y-6 animate-in fade-in duration-300">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <h2 className="text-xl md:text-2xl font-bold font-headline text-card-foreground tracking-tight">My Team</h2>
            <p className="text-sm text-muted-foreground mt-1 font-body">Manage your direct reports and team members.</p>
          </div>
          <Button onClick={() => setTaskModalOpen(true)} className="bg-indigo-600 hover:bg-indigo-700">
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {displayEmployees.map((emp: any) => (
              <EmployeeCard key={emp.id} employee={emp} basePath="/manager/my-team" />
            ))}
          </div>
        ) : (
          <div className="bg-card rounded-2xl shadow-sm border border-border overflow-hidden">
            <div className="hidden md:block overflow-x-auto w-full">
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-slate-50 text-sm">
                  {displayEmployees.map((emp: any, index: number) => (
                    <tr key={emp.id} className={`${index % 2 === 0 ? 'bg-card' : 'bg-slate-50/30'} hover:bg-muted transition-colors`}>
                      ...
                      <td className="py-4 px-6 text-right">
                        <Link href={`/manager/my-team/${emp.id}`}>
            ...
            {displayEmployees.length > 0 && (
              <div className="px-6 py-4 border-t border-border flex items-center justify-between bg-white/50">
                <span className="text-xs font-medium text-muted-foreground">Showing {displayEmployees.length} entries</span>
